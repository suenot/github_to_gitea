...
const organization: string | undefined = process.env.GITHUB_ORGANIZATION;
const giteaUrl: string | undefined = process.env.GITEA_URL;
const giteaToken: string | undefined = process.env.GITEA_TOKEN;
const giteaOrganization: string | undefined = process.env.GITEA_ORGANIZATION;
...
async function getRepositories(org: string) {
  ...
  while (hasMore) {
    try {
      ...
      repos = repos.concat(data.map((repo: any) => repo.clone_url));
      page++;
...
const addMirrors = async (repos: string[]) => {
  for (const repo of repos) {
    try {
      ...
      if ((error as any).response.data.message === 'The repository with the same name already exists.') {
